...
func TestLoadHostsConfigParsesExampleShape(t *testing.T) {
	...
	if err := os.WriteFile(path, []byte(faultFixtureYAML), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestFaultResolveTargetPresent(t *testing.T) {
	hosts := hostsConfig{"worker1": {Kill: "k", Restore: "r"}}
	a, err := faultResolveTarget(hosts, "worker1")
	if err != nil {
		t.Fatalf("faultResolveTarget: %v", err)
	}
...
func TestFaultResolveTargetAbsent(t *testing.T) {
	hosts := hostsConfig{"worker1": {Kill: "k", Restore: "r"}, "manager": {Kill: "k2", Restore: "r2"}}
	_, err := faultResolveTarget(hosts, "worker2")
	if err == nil {
...
func TestValidateFaultTargetMirrorsResolve(t *testing.T) {
	hosts := hostsConfig{"worker1": {Kill: "k", Restore: "r"}}
	if err := ValidateFaultTarget(hosts, "worker1"); err != nil {
		t.Fatalf("ValidateFaultTarget(present): %v", err)
	}
	if err := ValidateFaultTarget(hosts, "bogus"); err == nil {
		t.Fatal("ValidateFaultTarget(absent): want error")
	}
}

func TestFaultCondition(t *testing.T) {
	got := faultCondition("worker1", 60*time.Second, "events=on")
	want := "target=worker1;restore_after=1m0s;events=on"
	if got != want {
		t.Fatalf("faultCondition = %q, want %q", got, want)
	}
}

func TestFaultConditionZeroRestoreAfter(t *testing.T) {
	got := faultCondition("manager", 0, "events=off")
...
func TestFaultFalseOKEqualSpecsConvergedIsNotFalseOK(t *testing.T) {
	s := spec.ServiceSpec{Name: "fault_web1", Image: "nginx:1.24-alpine", Replicas: 1}
	if got := faultFalseOK(s, s, true); got {
		t.Fatal("equal specs with converged=true must not be false_ok")
	}
}

func TestFaultFalseOKDifferingSpecsConvergedIsFalseOK(t *testing.T) {
	desired := spec.ServiceSpec{Name: "fault_web1", Image: "nginx:1.24-alpine", Replicas: 1}
	observed := spec.ServiceSpec{Name: "fault_web1", Image: "nginx:1.25-alpine", Replicas: 1}
	if got := faultFalseOK(desired, observed, true); !got {
		t.Fatal("differing specs with converged=true must be false_ok")
	}
}

func TestFaultFalseOKDifferingSpecsNotConvergedIsNotFalseOK(t *testing.T) {
	desired := spec.ServiceSpec{Name: "fault_web1", Image: "nginx:1.24-alpine", Replicas: 1}
	observed := spec.ServiceSpec{Name: "fault_web1", Image: "nginx:1.25-alpine", Replicas: 2}
	if got := faultFalseOK(desired, observed, false); got {
		t.Fatal("never-converged run must never be false_ok, regardless of mismatch")
	}
}

func TestFaultRunShellRejectsEmptyCommand(t *testing.T) {
	// The empty check runs before ctx is touched, so a nil context is safe
	// here and this stays free of a live shell dependency.
	if err := faultRunShell(nil, ""); err == nil {
