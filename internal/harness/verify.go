...
// VerifyCases are the valid --case values.
var VerifyCases = []string{"ok", "unsigned", "wrong-identity", "no-attestation", "tag-repoint"}

// verifyVariant maps a static --case to the pipeline image variant
...
var verifyVariant = map[string]string{
	"ok":             "ok-signed",
	"unsigned":       "unsigned",
...
// VerifyConfig configures one gate-verification run.
type VerifyConfig struct {
	...
	Case     string // one of VerifyCases
...
func verifyCondition(cfg VerifyConfig) string {
	return fmt.Sprintf("case=%s;mode=%s;%s", cfg.Case, cfg.Mode, cfg.Label)
}

func verifyServiceName(base string, run int) string {
	return fmt.Sprintf("%s-%d", base, run)
}

func verifyRow(cfg VerifyConfig, run int, tStart, tEnd time.Time, outcome, detail string) Row {
	return Row{
		Scenario: "verify", Condition: verifyCondition(cfg), Run: run,
		TStart: tStart, TEnd: tEnd, DurationMS: tEnd.Sub(tStart).Milliseconds(),
		Outcome: outcome, Detail: detail,
	}
}

func verifyErrorRow(cfg VerifyConfig, run int, detail string) Row {
	now := time.Now()
	return verifyRow(cfg, run, now, now, "error", detail)
}

// verifyImage returns the pipeline image reference cfg.Case's stack service
// should point at.
func verifyImage(cfg VerifyConfig) (string, error) {
	if cfg.Case == "tag-repoint" {
		return fmt.Sprintf("%s/swarmgate-test/tag-repoint:v1", cfg.Registry), nil
	}
	variant, ok := verifyVariant[cfg.Case]
	if !ok {
		return "", fmt.Errorf("verify: unknown case %q", cfg.Case)
	}
	return fmt.Sprintf("%s/swarmgate-test/%s:v1", cfg.Registry, variant), nil
}

// verifySigTag renders digest (sha256:...) in the tag form cosign's new bundle
// format uses to locate a signed image's companion bundle artifact within
// its own repository.
func verifySigTag(digest string) string {
	return "sha256-" + strings.TrimPrefix(digest, "sha256:")
}

// verifyAPI is the slice of the Docker client the tag-repoint case needs, to
...
func newVerifyClient(host string) (verifyAPI, error) {
...
// verifyDeployedDigest inspects the live service and returns the digest
...
func verifyDeployedDigest(ctx context.Context, api verifyAPI, service string) (string, error) {
...
type verifyRunner struct {
	cfg VerifyConfig
	api verifyAPI // nil unless cfg.Case == "tag-repoint"
}

func newVerifyRunner(cfg VerifyConfig) (*verifyRunner, error) {
	if cfg.Case != "tag-repoint" {
		return &verifyRunner{cfg: cfg}, nil
	}
	api, err := newVerifyClient(cfg.DockerHost)
	if err != nil {
		return nil, err
	}
	return &verifyRunner{cfg: cfg, api: api}, nil
}

func (r *verifyRunner) run(runIdx int) Row {
	cfg := r.cfg
	...
	bareService := verifyServiceName(cfg.Service, runIdx)
	qualifiedService := spec.ServiceName(cfg.Stack, bareService)

	image, err := verifyImage(cfg)
	if err != nil {
		return verifyErrorRow(cfg, runIdx, err.Error())
	}
	...
	if cfg.Case == "tag-repoint" {
		...
		if err != nil {
			return verifyErrorRow(cfg, runIdx, fmt.Sprintf("seed tag-repoint: digest: %v", err))
		}
		if err := crane.Copy(seedRef, image); err != nil {
			return verifyErrorRow(cfg, runIdx, fmt.Sprintf("seed tag-repoint: %v", err))
		}
		...
		if err := crane.Copy(sigSrc, sigDst); err != nil {
			return verifyErrorRow(cfg, runIdx, fmt.Sprintf("seed tag-repoint signature: %v", err))
		}
	}
	...
	if err != nil {
		return verifyErrorRow(cfg, runIdx, fmt.Sprintf("tail events: %v", err))
	}

	stackPath := filepath.Join(cfg.Repo, cfg.Stack+".yaml")
	stack := scaleStackYAML([]scaleService{{Name: bareService, Image: image}})
	if err := os.WriteFile(stackPath, []byte(stack), 0o644); err != nil {
		return verifyErrorRow(cfg, runIdx, fmt.Sprintf("write stack file: %v", err))
	}
	sha, err := gitCommitAndPush(cfg.Repo, cfg.Stack+".yaml", fmt.Sprintf("verify run %d", runIdx), cfg.Push)
	if err != nil {
		return verifyErrorRow(cfg, runIdx, err.Error())
	}
	tStart := time.Now()
...
func (r *verifyRunner) runStaticCase(ctx context.Context, runIdx int, service string, tStart time.Time, events <-chan telemetry.Event, sha string) Row {
	cfg := r.cfg
	...
	if err != nil {
		return verifyRow(cfg, runIdx, tStart, time.Now(), "timeout", "verify: "+err.Error())
	}
	...
	if gotOutcome != wantOutcome {
		...
		return verifyRow(cfg, runIdx, tStart, ve.T, "error",
			fmt.Sprintf("verify=%s;want=%s;reason=%s", gotOutcome, wantOutcome, reason))
	}
	if cfg.Case != "ok" {
		return verifyRow(cfg, runIdx, tStart, ve.T, "ok", "verify="+gotOutcome)
	}
	...
	if err != nil {
		return verifyRow(cfg, runIdx, tStart, time.Now(), "timeout", "verify=pass;converged=timeout")
	}
	return verifyRow(cfg, runIdx, tStart, ce.T, "ok", "verify=pass;converged=true")
}
...
// tag at apply time.
func (r *verifyRunner) runTagRepoint(ctx context.Context, runIdx int, service, sha string, events <-chan telemetry.Event, tStart time.Time) Row {
	cfg := r.cfg
	...
	if err != nil {
		return verifyRow(cfg, runIdx, tStart, time.Now(), "timeout", "resolve: "+err.Error())
	}
	resolvedDigest, _ := re.Fields["digest"].(string)
	if resolvedDigest == "" {
		return verifyRow(cfg, runIdx, tStart, re.T, "error", "resolve event carried no digest")
	}
	...
	if err := crane.Copy(unsignedRef, tagRepointRef); err != nil {
		return verifyRow(cfg, runIdx, tStart, re.T, "error", fmt.Sprintf("retag: %v", err))
	}
	...
	ce, err := awaitFromChannel(ctx, events, convMatch)
	if err != nil {
		return verifyRow(cfg, runIdx, tStart, time.Now(), "timeout", fmt.Sprintf("resolved=%s;converged=timeout", resolvedDigest))
	}

	deployed, err := verifyDeployedDigest(context.Background(), r.api, service)
	if err != nil {
		return verifyRow(cfg, runIdx, tStart, ce.T, "error", fmt.Sprintf("resolved=%s;inspect_error=%v", resolvedDigest, err))
	}
	if deployed != resolvedDigest {
		return verifyRow(cfg, runIdx, tStart, ce.T, "error",
			fmt.Sprintf("resolved=%s;deployed=%s;match=false", resolvedDigest, deployed))
	}
	return verifyRow(cfg, runIdx, tStart, ce.T, "ok",
		fmt.Sprintf("resolved=%s;deployed=%s;match=true", resolvedDigest, deployed))
}

// RunVerify executes a gate-verification run: n iterations, each pointing a freshly
// (uniquely) named stack service at the image cfg.Case selects, pushing,
// and recording the observed gate outcome.
func RunVerify(cfg VerifyConfig, n int, out *CSVWriter) error {
	if cfg.Case != "tag-repoint" {
		if _, ok := verifyVariant[cfg.Case]; !ok {
			return fmt.Errorf("verify: unknown case %q", cfg.Case)
		}
	}
	r, err := newVerifyRunner(cfg)
	if err != nil {
		return err
	}
